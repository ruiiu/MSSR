...
class AlgorithmConfig:
    ...
    # Visual-aware SPO: Dual-stream advantage reweighting for multimodal learning
    # This focuses learning on tokens where visual input has high influence
    spo_enable_visual_reweighting: bool = False
    """enable visual-aware advantage reweighting (requires multimodal model).
    When enabled, computes dual-stream logprobs (full vs text-only) to identify
    tokens where visual input has high influence, then reweights advantages accordingly."""
    spo_visual_reweight_strength: float = 1.0
    """strength of visual reweighting (0.0=disabled, 1.0=full, 2.0=aggressive).
    Higher values amplify the gradient signal for visual-influenced tokens."""
    spo_visual_temperature: float = 1.0
    """temperature for softmax when computing visual influence weights.
    Lower temp (e.g., 0.5) focuses more on highest-influence tokens,
    higher temp (e.g., 2.0) spreads weights more uniformly."""
    spo_visual_threshold: float = 0.1
    """minimum logprob difference to consider as 'visual helps' (default 0.1).
    Only tokens with logprob_diff > threshold get non-zero weights.
    IMPORTANT: Actual values depend on your model! Monitor visual/pct_amplified_tokens.
    For strong visual models (Qwen2.5-VL, etc), you may need much higher thresholds:
    - 1.0-2.0: For models with moderate visual influence
    - 2.0-3.0: For models with strong visual influence [Often needed]
    - 3.0-5.0: For models with very strong visual influence
    Target: 10-30% of tokens amplified. Tune based on visual/pct_amplified_tokens metric.
    This prevents amplifying tokens where visual input has negligible effect."""
    
    # Visual reweighting annealing: gradually phase out visual reweighting during training
    spo_visual_annealing: bool = False
    """enable annealing for visual reweighting. When enabled, visual reweighting probability
    starts high and gradually decreases to zero over training, allowing model to learn
    visual grounding early then rely on learned understanding later."""
    spo_visual_annealing_start_prob: float = 1.0
    """initial probability of using visual reweighting (1.0 = always use, 0.0 = never use).
    Typically set to 1.0 to start with full visual reweighting."""
    spo_visual_annealing_end_prob: float = 0.0
    """final probability of using visual reweighting after annealing.
    Typically set to 0.0 to phase out visual reweighting completely."""
    spo_visual_annealing_start_step: int = 0
    """training step to start annealing (0 = from beginning).
    Annealing ends at trainer.max_steps (no separate end_step needed)."""
    
    # Entropy-based advantage shaping for SPO
    ...
    Only applies when spo_enable_visual_reweighting=True (requires dual-stream setup).
    Computes KL(text_only || multimodal) where text-only is the reference/anchor policy.
