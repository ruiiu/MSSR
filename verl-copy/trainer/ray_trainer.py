...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0, 
                     spo_value_tracker=None, spo_config=None, visual_metrics_out=None):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        # Extract visual weights if available (for visual-aware advantage reweighting)
        visual_weights = data.batch.get("visual_influence_weights", None)
        # Ensure visual_weights is a tensor, not a TensorDict
        if visual_weights is not None and hasattr(visual_weights, 'batch'):
            # If it's a TensorDict, extract the actual tensor
            visual_weights = visual_weights.batch.get("visual_influence_weights", None)
        visual_reweight_strength = spo_config.spo_visual_reweight_strength if (spo_config and hasattr(spo_config, 'spo_visual_reweight_strength')) else 0.0
        
        # If reweight_strength is 0.0, don't use visual weights
        if visual_reweight_strength == 0.0:
            visual_weights = None
        
        # Extract entropy shaping parameters (for entropy-based advantage shaping)
        ...
        # Extract log_probs for entropy computation (use old_log_probs if available)
        log_probs_for_entropy = old_log_probs if use_entropy_shaping else None
        
        # Compute SPO advantages with optional visual reweighting and entropy shaping
        result = core_algos.compute_spo_advantage(
            ...
            visual_reweight_strength=visual_reweight_strength,
        ...
        if len(result) == 3:
            ...
            if visual_metrics_out is not None:
                visual_metrics_out.update(metrics)
        else:
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    ...
                    # Compute visual influence weights for SPO with visual-aware reweighting
                    # Always clear visual weights first to avoid stale data
                    if "visual_influence_weights" in batch.batch:
                        batch.batch.pop("visual_influence_weights", None)
                    # if "logprob_diff" in batch.batch:
                    #     batch.batch.pop("logprob_diff", None)
                    
                    if (self.config.algorithm.adv_estimator == AdvantageEstimator.SPO and 
                        hasattr(self.config.algorithm, 'spo_enable_visual_reweighting') and
                        self.config.algorithm.spo_enable_visual_reweighting):
                        
                        # Check if we should use visual reweighting this step (annealing)
                        use_visual_reweighting = True
                        visual_annealing_prob = 1.0
                        
                        if (hasattr(self.config.algorithm, 'spo_visual_annealing') and 
                            self.config.algorithm.spo_visual_annealing):
                            # Compute current annealing probability
                            current_step = self.global_step
                            start_step = self.config.algorithm.spo_visual_annealing_start_step
                            end_step = self.config.trainer.max_steps  # Use trainer.max_steps as end step
                            start_prob = self.config.algorithm.spo_visual_annealing_start_prob
                            end_prob = self.config.algorithm.spo_visual_annealing_end_prob
                            
                            if current_step < start_step:
                                visual_annealing_prob = start_prob
                            elif current_step >= end_step:
                                visual_annealing_prob = end_prob
                            else:
                                # Linear interpolation
                                progress = (current_step - start_step) / (end_step - start_step)
                                visual_annealing_prob = start_prob + (end_prob - start_prob) * progress
                            
                            # Decide whether to use visual reweighting this step
                            import random
                            use_visual_reweighting = random.random() < visual_annealing_prob
                            
                            # Log annealing probability
                            metrics["visual/annealing_prob"] = visual_annealing_prob
                            metrics["visual/use_reweighting"] = float(use_visual_reweighting)
                        
                        if use_visual_reweighting:
                            with timer("visual_influence", timing_raw):
                                ...
                                # Compute visual influence weights
                                visual_weights, visual_influence_metrics = core_algos.compute_visual_influence_weights_log(
                                    full_modality_log_probs=full_log_probs,
                                    text_only_log_probs=text_only_logprobs_tensor,
                                    response_mask=batch.batch["response_mask"],
                                    temperature=self.config.algorithm.spo_visual_temperature if hasattr(self.config.algorithm, 'spo_visual_temperature') else 1.0,
                                    visual_threshold=self.config.algorithm.spo_visual_threshold if hasattr(self.config.algorithm, 'spo_visual_threshold') else 0.1
                                )

                                # Compute KL divergence between text-only and multimodal streams if enabled
                                if self.config.algorithm.text_kl_enabled:
                                ...
                                # Store visual weights in batch for advantage computation
                                ...
                                batch.batch["visual_influence_weights"] = visual_weights
                                # batch.batch["logprob_diff"] = logprob_diff  # Store for sign-based reweighting
                                
                                # Log visual influence metrics
                                metrics.update(visual_influence_metrics)
                                metrics["visual/annealing_skipped"] = 0.0
                        else:
                            # Skip visual reweighting this step (single-stream)
                            # Don't set visual_influence_weights - it will be None from the pop above
                            metrics["visual/annealing_skipped"] = 1.0
                    ...
                    batch = compute_advantage(
                        ...
                        visual_metrics_out=visual_metrics_dict
                    )
                    
                    # Log visual reweighting metrics if computed
                    if visual_metrics_dict:
                        metrics.update(visual_metrics_dict)
