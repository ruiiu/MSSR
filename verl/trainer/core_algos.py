...
class MVSRValueTracker:
    ...
    def _get_adaptive_rho(self) -> float:
        ...
        if len(self.recent_kl_values) == 0:
            # No KL data yet, use middle value
            return (self.rho_min + self.rho_max) / 2
        
        # Compute mean KL from recent history
        mean_kl = np.mean(self.recent_kl_values[-self.kl_window_size:])
        ...
        if mean_kl > self.target_kl:
            ...
            ratio = min(mean_kl / (1 * self.target_kl), 1.0)
...
def compute_reinforce_plus_plus_outcome_advantage(
    ...
    Args:
        ...
        use_entropy_shaping: `(bool)`
            Whether to apply entropy-based advantage shaping for exploration
...
def compute_mvsr_advantage(
    ...
    6. Apply entropy-based advantage shaping if enabled
       - Encourages exploration by adding entropy-based bonus to advantages
       - Formula: ψ(H_t) = min(α·H_t^detach, |A_t|/κ)
    7. Convert scores to binary outcomes {0,1} for value tracker update
    8. Calculate per-prompt rho based on policy change (if enabled)
    9. Update value tracker with binary outcomes using Beta-Bernoulli formula
    ...
    Returns:
        ...
        metrics: `(Dict[str, float])` [optional]
            Metrics about entropy shaping (only if enabled)
    """
    ...
    if entropy_metrics:
        return advantages, returns, entropy_metrics
    else:
        return advantages, returns
